 ...
 */
const querySingleSensorData = async (
  code: string,
  startDate: string,
  endDate: string,
  sensor: 1 | 2
): Promise<QuerySensorResult[]> => {
 ...
 * 查询传感器数据（支持多个items）
 * @param params 查询参数
 * @returns 按items数组顺序返回的查询结果数组，每个元素对应一个item的查询结果
 */
export const querySensorData = async (params: QuerySensorParams): Promise<QuerySensorResult[][]> => {
  try {
    const { items, startDate, endDate } = params;


    // 按顺序查询每个item，保持数组顺序
    const results: QuerySensorResult[][] = [];
    for (const item of items) {


      const itemResult = await querySingleSensorData(
        item.code,
        startDate,
        endDate,
        item.sensor
